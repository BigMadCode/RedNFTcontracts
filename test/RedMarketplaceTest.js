...
contract("Marketplace Test", function (accounts) {
  ...
  it("#4 it is possible to make an offer", async () => {
    ...
    let allowance = await redInstance.allowance(
      anotherAccount,
      instance.address
    );
  ...
  it("#7 it is possible to accept offer", async () => {
    ...
    await instance.acceptOffer(nftInstance.address, offerId, {
      from: recipient,
    });
  ...
  it("#8 it is possible to buy listed item immediately by paying amount >= asking price", async () => {
    ...
    await instance.buyNow(nftInstance.address, listingId, 20, {
      from: anotherAccount,
    });
  ...
  it("#8 it is possible to sell after buying", async () => {
    let instance = this.redMarketplace;
    let redInstance = await Token.deployed();
    let nftInstance = await NFTContract.deployed();
    console.log("deployer " + deployerAccount);
    console.log("recipient " + recipient);
    console.log("another " + anotherAccount);
    let item = await instance.getListingById(0);
    await expect(
      redInstance.increaseAllowance(instance.address, 100000, {
        from: recipient,
    ...
    console.log(item);
    const ow = await nftInstance.ownerOf(item.tokenId);
    let nftBalanceRecipient = await nftInstance.balanceOf(recipient);
    console.log("Owner of " + ow);
    ...
    await nftInstance.approve(instance.address, item.tokenId, {
      from: anotherAccount,
    });
    await instance.acceptOffer(nftInstance.address, offerId, {
      from: anotherAccount,
