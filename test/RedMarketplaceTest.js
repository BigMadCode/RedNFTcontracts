...
contract('Marketplace Test', function (accounts) {
	...
	it('#2 should list token lisitngs', async () => {
		...
		console.log(balanceRecipient.toString() + ' this is balance of Recipient');
		...
		let NFT = await expect(nftInstance.safeMint(recipient, 'www.redeyedcollection.com/')).to.eventually.be
			.fulfilled;
		let nftTokenId = await NFT;
		//console.log(NFT);
		let balanceRecipient1 = await nftInstance.balanceOf(recipient);
		//console.log(nftTokenId.toString());
		console.log(balanceRecipient1.toString() + ' this is balance of Recipient1');
		expect(instance.listItem(NFT.tokenId, NFT.address, askingPrice, 1, recipient.address)).to.eventually.be
			.fulfilled;
	});
